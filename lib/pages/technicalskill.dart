...
class _ts2State extends State<ts2> {
  ...
  void _loadData() async {
    final result = await dbhelper().getquestion_ts();
    final resultint = await dbhelper().readint('finalvalue_ts');
    final resultscore = await dbhelper().score("matchscore_ts");
    final resulthighscore = await dbhelper().highscore("totalscore_ts");
  ...
  update() async {
    ...
    await dbhelper().updateid(increment, 'finalvalue_ts');
    score = score + win;
    await dbhelper().updatescore(score, 'matchscore_ts');
    int win1 = 0;
    if (hscore < score)
      win1 = score;
    else {
    ...
    await dbhelper().updatehighscore(win1, 'totalscore_ts');
  }

  updatehighscore() async {
    await dbhelper().updatescore(0, 'matchscore_ts');
  }

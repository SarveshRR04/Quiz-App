...
class _sportState extends State<sport> {
  ...
  void _loadData() async {
    final result = await dbhelper().getquestion_sq();
    final resultint = await dbhelper().readint('finalvalue_sq');
    final resultscore = await dbhelper().score("matchscore_sq");
    final resulthighscore = await dbhelper().highscore("totalscore_sq");
  ...
  update() async {
    ...
    await dbhelper().updateid(increment, 'finalvalue_sq');
    score = score + win;
    await dbhelper().updatescore(score, 'matchscore_sq');
    int win1 = 0;
    if (hscore < score)
      win1 = score;
    else {
    ...
    await dbhelper().updatehighscore(win1, 'totalscore_sq');
  }

  updatehighscore() async {
    await dbhelper().updatescore(0, 'matchscore_sq');
  }

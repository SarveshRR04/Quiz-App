...
class historystless extends StatelessWidget {
  const historystless({super.key});
...
class _historyState extends State<history> {
  ...
  void _loadData() async {
    final result = await dbhelper().getquestion_ht();
    final resultint = await dbhelper().readint('finalvalue_ht');
    final resultscore = await dbhelper().score("matchscore_ht");
    final resulthighscore = await dbhelper().highscore("totalscore_ht");
  ...
  update() async {
    ...
    await dbhelper().updateid(increment, 'finalvalue_ht');
    score = score + win;
    await dbhelper().updatescore(score, 'matchscore_ht');
    int win1 = 0;
    if (hscore < score)
      win1 = score;
    else {
    ...
    await dbhelper().updatehighscore(win1, 'totalscore_ht');
  }

  updatehighscore() async {
    await dbhelper().updatescore(0, 'matchscore_ht');
  }
